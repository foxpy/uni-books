...
bool database_add_game(db* database, char const* gamename, char const* author, char** err) {
    UNUSED(err);
    char const* query = "INSERT INTO Games(name, exists_flag, author) VALUES(?1, TRUE, ?2);";
    sqlite3_stmt* stmt;
